  function runCardScripts() {
    ...
    function getVerbForms(verb) {
      ...
      // Handle verbs ending in 'e' (e.g., revise -> revised, revising)
      if (verb.endsWith('e')) {
        const baseWithoutE = verb.slice(0, -1);
        return [
          verb, // revise
          `${verb}s`, // revises
          `${baseWithoutE}ed`, // revised
          `${baseWithoutE}ing` // revising
        ];
      }

      // Handle verbs with CVC pattern (e.g., rub -> rubbed, rubbing)
      const doubled = isCVC ? verb + lastChar : verb;
      const baseWithoutE = verb;

      return [
        verb, // rub
        `${verb}s`, // rubs
        `${doubled}ed`, // rubbed
        `${baseWithoutE}ing`, // rubbing
        `${doubled}ing` // rubbing
      ];
    }
